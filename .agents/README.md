...
5. **Projects (Where real product repos live)**: Located in [projects/](file:///d:/eddie-agents/coding-agent-workspace/.agents/projects/). Config files containing paths to target repositories.
...
7. **Handoffs (Session handoffs)**: Located in [handoffs/](file:///d:/eddie-agents/coding-agent-workspace/.agents/handoffs/). Logs to transfer status, context, and next steps between agent sessions.
8. **Sessions (Session logs)**: Located in [sessions/](file:///d:/eddie-agents/coding-agent-workspace/.agents/sessions/). Historical records of workspace execution, including `session-log.md`.
...
- **`projects/`**: Reference JSONs pointer to external repos.
- **`handoffs/`**: Shift checklist logs.
- **`sessions/`**: Cumulative session logs (`session-log.md`).
